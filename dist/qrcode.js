...
/***/ function(module, exports, __webpack_require__) {
	...
	FormatInformation.numBitsDiffering = function (a, b) {
	    ...
	    return BITS_SET_IN_HALF_BYTE[a & 0x0F] + BITS_SET_IN_HALF_BYTE[(URShift(a, 4) & 0x0F)] +
	        BITS_SET_IN_HALF_BYTE[(URShift(a, 8) & 0x0F)] + BITS_SET_IN_HALF_BYTE[(URShift(a, 12) & 0x0F)] +
	        BITS_SET_IN_HALF_BYTE[(URShift(a, 16) & 0x0F)] + BITS_SET_IN_HALF_BYTE[(URShift(a, 20) & 0x0F)] +
	        BITS_SET_IN_HALF_BYTE[(URShift(a, 24) & 0x0F)] + BITS_SET_IN_HALF_BYTE[(URShift(a, 28) & 0x0F)];
	};
...
/***/ function(module, exports, __webpack_require__) {
	...
	function Version(versionNumber, alignmentPatternCenters, ecBlocks1, ecBlocks2, ecBlocks3, ecBlocks4) {
	    ...
	    this.buildFunctionPattern = function () {
	        ...
	        for (var x = 0; x < max; x++) {
	            var ix = this.alignmentPatternCenters[x] - 2;
	            for (var y = 0; y < max; y++) {
	                if ((x === 0 && (y === 0 || y === max - 1)) || (x === max - 1 && y === 0)) {
	                    // No alignment patterns near the three finder paterns
	                    continue;
	                }
	                bitMatrix.setRegion(this.alignmentPatternCenters[y] - 2, ix, 5, 5);
	            }
	        }
...
/***/ function(module, exports, __webpack_require__) {
	...
	var AlignmentPattern = __webpack_require__(8);
	...
	function AlignmentPatternFinder(image, startX, startY, width, height, moduleSize) {
	    ...
	    this.crossCheckVertical = function (startI, centerJ, maxCount, originalStateCountTotal) {
	        ...
	        var i = startI;
	        while (i >= 0 && image[centerJ + i * image.width] && stateCount[1] <= maxCount) {
	            stateCount[1]++;
	            i--;
	        }
	        ...
	        while (i >= 0 && !image[centerJ + i * image.width] && stateCount[0] <= maxCount) {
	            stateCount[0]++;
	            i--;
	        }
	        ...
	        i = startI + 1;
	        while (i < maxI && image[centerJ + i * image.width] && stateCount[1] <= maxCount) {
	            stateCount[1]++;
	            i++;
	        }
	        ...
	        while (i < maxI && !image[centerJ + i * image.width] && stateCount[2] <= maxCount) {
	            stateCount[2]++;
	            i++;
	        }
	    ...
	    this.find = function () {
	        ...
	        for (var iGen = 0; iGen < height; iGen++) {
	            ...
	            while (j < maxJ && !image[j + image.width * i]) {
	                j++;
	            }
	            var currentState = 0;
	            while (j < maxJ) {
	                if (image[j + i * image.width]) {
	                    // Black pixel
	                    if (currentState === 1) {
...
/***/ function(module, exports, __webpack_require__) {
	...
	var DataMask = __webpack_require__(11);
...
/***/ function(module, exports, __webpack_require__) {
	...
	var AlignmentPatternFinder = __webpack_require__(7);
	var GridSampler = __webpack_require__(16);
	var Version = __webpack_require__(6);
	var FinderPatternFinder = __webpack_require__(15);
...
/***/ function(module, exports) {
	...
	function FinderPatternFinder() {
	    ...
	    this.crossCheckVertical = function (startI, centerJ, maxCount, originalStateCountTotal) {
	        ...
	        // Start counting up from center
	        var i = startI;
	        while (i >= 0 && image[centerJ + i * image.width]) {
	            stateCount[2]++;
	            i--;
	        }
	        if (i < 0) {
	        ...
	        // Now also count down from center
	        i = startI + 1;
	        while (i < maxI && image[centerJ + i * image.width]) {
	            stateCount[2]++;
	            i++;
	        }
	        if (i === maxI) {
	        ...
	        while (i < maxI && !image[centerJ + i * image.width] && stateCount[3] < maxCount) {
	            stateCount[3]++;
	            i++;
	        }
	        ...
	        while (i < maxI && image[centerJ + i * image.width] && stateCount[4] < maxCount) {
	            stateCount[4]++;
	            i++;
	        }
	    ...
	    this.crossCheckHorizontal = function (startJ, centerI, maxCount, originalStateCountTotal) {
	        ...
	        var j = startJ;
	        while (j >= 0 && image[j + centerI * image.width]) {
	            stateCount[2]++;
	            j--;
	        }
	        if (j < 0) {
	        ...
	        while (j >= 0 && !image[j + centerI * image.width] && stateCount[1] <= maxCount) {
	            stateCount[1]++;
	            j--;
	        }
	        ...
	        while (j >= 0 && image[j + centerI * image.width] && stateCount[0] <= maxCount) {
	            stateCount[0]++;
	            j--;
	        }
	        if (stateCount[0] > maxCount) {
	        ...
	        j = startJ + 1;
	        while (j < maxJ && image[j + centerI * image.width]) {
	            stateCount[2]++;
	            j++;
	        }
	        if (j === maxJ) {
	        ...
	        while (j < maxJ && !image[j + centerI * image.width] && stateCount[3] < maxCount) {
	            stateCount[3]++;
	            j++;
	        }
	        ...
	        while (j < maxJ && image[j + centerI * image.width] && stateCount[4] < maxCount) {
	            stateCount[4]++;
	            j++;
	        }
	        if (stateCount[4] >= maxCount) {
	                        ...
	                        if (currentState === 4) {
	                            ...
	                            if (this.foundPatternCross(stateCount)) {
	                                ...
	                                if (confirmed) {
	                                    ...
	                                    do {
	                                        j++;
	                                    } while (j < maxJ && !image[j + i * image.width]);
	    ...
	    for (var y = 0; y < dimension; y++) {
	        ...
	        try {
	            for (x = 0; x < max; x += 2) {
	                ...
	                var bit = image[Math.floor(points[x]) + image.width * Math.floor(points[x + 1])];
	                image.imagedata.data[xpoint] = bit ? 255 : 0;
